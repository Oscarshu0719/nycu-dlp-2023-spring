...
class Solver(object): 
    def __init__(self, config: dict) -> None:
        ...
        for setup in self.models:
            ...
            self.train(model, optim, scheduler, criterion)
            
        # Plot.
        path = (f'{config.model}_{self.optim}'
        ...
        id = (f'{config.model}_{self.optim}'
            f'_{config.batch_size}_{self.lr}_{self.dropout}')
        with open(config.path_acc, 'a') as file:
    ...
    def train(self, model, optim, scheduler, criterion) -> None:
