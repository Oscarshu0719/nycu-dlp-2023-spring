...
public:
	 ...
	 */
	void make_statistic(
		size_t n, const board& b, int score, int unit = 1000, std::string path_output = "") {
		...
		if (n % unit == 0) { // show the training process
			...
			// Save means to file each iteration.
			if (path_output.length() != 0) {
				std::ofstream ofs;
				ofs.open(path_output, std::ios_base::app);

				if (!ofs.is_open())
					error << "Failed to open file \"" << path_output << "\"." << std::endl;
				else
					for (auto& ele : scores)
						ofs << ele << ",";
...
int main(int argc, const char* argv[]) {
	...
	// Logging.
	const char* DIR_OUTPUT = "output/";
	std::string PATH_OUTPUT = "output.txt";
	std::string dir_output;
	bool is_saving = true;
	int iter_saving = 10000;

	struct stat info;
	if (stat(DIR_OUTPUT, &info) != 0) {
		error << "[ERR] Cannot access \"" << DIR_OUTPUT << "\".\n" << std::endl;
		dir_output = "./";
	} else
	...
	tdl.load("tmp/output/weight_240000.bin");
	...
	for (size_t n = 1; n <= total; n++) {
		...
		tdl.make_statistic(n, b, score, 1000, is_saving ? dir_output + PATH_OUTPUT : "");
		path.clear();

		if (n % iter_saving == 0)
			tdl.save(dir_output + "weight_" + std::to_string(n) + ".bin");
	}
